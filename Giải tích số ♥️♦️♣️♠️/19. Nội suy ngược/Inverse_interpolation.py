...
class Inverse_interpolation:
    ...
    def fromFileInput(cls):
        ...
        if len(table_x) == 0 or len(table_x) == 1:
            colored("xTableSizeError: xTable doesn't have enough element", 'pink')
            sys.exit() 
        ...
        condition_error = {
            ...
            condition_yTableSize  : colored(f"TableSizeError: xTable has {len(table_x)} element(s), while yTable has {len(table_y)} element(s)", 'pink'),
            condition_xIncreasing : colored("xIncreasingError: xTable must have increasing value from left to right"                           , 'pink'),
            condition_xJumpStep   : colored("xJumpStepError: Distance between two consecutive value are not the same"                          , 'pink'),
        }.get(False) 
        
        if condition_error == None:
            print(colored("All condition satisfied! Printing the result...", 'green'))
        else:
    ...
    def __root(self):
        ...
        for i in range(len(mono_par)):
            ...
            print(colored(f"\nFinding the {ordinal(i + 1)} solution", 'green'))
